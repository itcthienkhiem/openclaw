...
import s from "./StartChatModelBadge.module.css";
...
export type StartChatModelBadgeProps = {
  modelLabel: string | null;
  /** Local-model mode: green dot when llama.cpp server is running. */
  showLocalServerRunning: boolean;
  onOpenSettings: () => void;
};

export function StartChatModelBadge(props: StartChatModelBadgeProps) {
  const { modelLabel, showLocalServerRunning, onOpenSettings } = props;
  if (!modelLabel?.trim()) {
    return null;
  }
  ...
  return (
    <div className={s.wrap}>
      {showLocalServerRunning ? (
        <span
          className={s.localDot}
          role="status"
          aria-label="Local model server running"
          title="Local model server running"
        />
      ) : null}
      <span className={s.modelName} title={modelLabel}>
        {modelLabel}
      </span>
      <button
        type="button"
